...
{
    public class OptimisticMovieRatingProvider: IMovieRatingProvider
    {
        private Dictionary<string, decimal> _ratings;
        ...
        public IDictionary<string, decimal> GetMovieRatings(IEnumerable<string> movieTitles)
        {
            ...
            if (_ratings == null)
            {
                var ratings = movieTitles
                    .Distinct()
                    .Select(title => new KeyValuePair<string, decimal>(title, (decimal)random.Next(80, 100) / 10));
